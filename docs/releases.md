...
- 機能増加に従って巨大化し続けたヘルプウィンドウの画面サイズを、小さく変更しました。メニューにスクロールを追加。
- ショートカットキーが別ウィンドウを開いた状態でもメニューが反応する(機能はしていなかった)問題の修正。
...
- 機能以外の修正
  ...
  - アプリサイズが肥大化。速度はやや改善。
    - サイズ削減処理が入っているが、それ以上に Electron フレームワークの大きさが増加。
  - 使用している画像フォーマットを変更。WebP、WebM を利用する。
    - 一部、WebP を使用するとクラッシュする箇所は PNG 形式のファイルを利用。
