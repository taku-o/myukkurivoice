# 0.7.8
* メッセージリストのファイル保存メッセージを選択すると、QuickLookで保存したファイルを表示する。
    * 音声ファイル、および、メッセージファイルが対象。
        * どのような音声を作ったか、後から確認するのに役に立つ想定。
        * 音声とテキストファイルを同時にQuickLookに表示することは現時点では実現できない。
    * 開いたQuickLookは、スペースキーで閉じられます。
* acceptFirstMouse(ウィンドウがアクティブでない時、最初のクリックを拾う) の初期設定値をONに変更。
    * この設定は環境設定から変更できます。
* ユーザー辞書編集機能への対応用の機能の追加。
    * カスタムScheme対応。
        * myukkurivoice://open 起動していなければアプリ起動
        * myukkurivoice://reload メイン画面、ユーザ辞書の再読み込み
        * myukkurivoice://quit 起動中アプリの終了
    * カスタム辞書が用意されていれば、それを利用して音声記号変換を行うように修正。
        * カスタム辞書を作る機能は別に用意します。
