...
| AquesTalk | vendor の `.framework`（AquesTalk1/2/10、AqKanji2Koe、AqUsrDic）。AquesTalk1 は Catalina 以降で iOS 版 bridge に切替 |
...
electron.remote 方針 ──┘              │
              ...
              FFI 置き換え → remote 置き換え → Electron 更新
                                       │
                                       ▼
              vendor 差し替え → arm64 パッケージング
                                       │
                                       ▼
              テスト基盤の置き換え → 署名・公証・MAS → 周辺ライブラリ更新
```

### 5.1 AquesTalk vendor（最大の外部ブロッカー）

2025-04-28 に、アクエストの macOS 用ライブラリ（AquesTalk1 / 2 / 10、AqKanji2Koe-A）が Apple Silicon 対応した。

現行 vendor との差分が見込まれる。

| 現行 | 新 SDK（調査時点） |
| --- | --- |
| `.framework` を asar unpacked に同梱 | `libAquesTalk10.dylib` 等の dylib。ベンダー提供物は arm64 + x86_64 の Universal Binary だが、アプリは arm64 のみ使う |
| 動作 OS は古い Mac を含む | macOS 11 以降 |
| AquesTalk1 は i386 / Catalina 以降は iOS 版 | Mac 版 AquesTalk1 自体が Apple Silicon 対応 |

発生作業:

- 新 SDK の入手、ライセンス、API、ヘッダ、辞書・phont の差分確認
- vendor プライベートリポジトリ（myukkurivoice-vendor）への取り込み
- 同梱パス変更（framework → dylib）。`gulpfile.package.js` の unpacked コピー、asar ignore、コード側の `DynamicLibrary` パス
- AquesTalk1 の i386 / iOS 切替（`maquestalk1` / `maquestalk1-ios`）を、新 Mac 版に合わせるかの判断。機能追加ではなく、現行の音声生成を新バイナリで成立させるための判断
- サンプリングレート等の仕様差分確認。現行アプリは AquesTalk 出力を 8000Hz として扱っている。新 AquesTalk10 マニュアルは 16kHz と記載。音声再生・録音・FCPX 連携に影響するため、実装前に実測する
- MAS サンドボックスでの dylib 配置（`@executable_path/../Frameworks` 等）

新 SDK が使えない、または API 非互換が大きい場合は、ここで止めて設計者に連絡する。実装を勝手に変えない。
...
1. 新 AquesTalk SDK の入手可否、ライセンス、dylib パス、関数シグネチャ、辞書・phont、サンプリングレート
2. 選ぶ FFI（koffi 想定）で、現行の Synthe / FreeWave / 構造体渡しが再現できるか
3. 到達 Electron の版。公証、選んだ FFI、macOS 27 での起動
4. `@electron/remote` で現行の `remote.app` / `getGlobal('appCfg')` / `getCurrentWindow` が足りるか
...
| 順 | 内容 | 依存 |
| --- | --- | --- |
| 1 | vendor / AquesTalk 新 SDK 取り込みとパス変更 | 調査 1 |
| 2 | FFI 置き換え | 調査 2、スペック 1 のパス |
| 3 | `electron.remote` 置き換え | 調査 4 |
| 4 | Electron / Node 更新 | スペック 2 と 3、調査 3 |
...
| 領域 | 大きさ | 理由 |
| --- | --- | --- |
| AquesTalk vendor | 大 | 外部成果物。形式変更の可能性。音声仕様の差分 |
...
- 新 AquesTalk SDK のサンプリングレートや声種が現行と違うと、録音ファイルや FCPX 連携の見え方が変わる
- AquesTalk1 新 Mac 版と、現行の iOS 版切替・声種 `m1` の扱いが一致しない
...
1. AquesTalk 新 SDK と FFI / Electron の調査スペック作成指示を出す（指示があるまで requirements.md 等は作らない）
