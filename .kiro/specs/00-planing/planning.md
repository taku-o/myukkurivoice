...
- **AquesTalk は Apple Silicon 対応版へ更新する。** 現行 vendor に arm64 は含まれない。開発ではアクエスト公開の評価版 SDK を使う（従来どおり）。製品配布用の有償ライセンスは、評価版では足りないと分かった時点で決める。評価版の再配布はしない。
...
| AquesTalk | vendor の `.framework`。実測で **arm64 なし**（AquesTalk1 は i386/ppc、それ以外は x86_64）。AquesTalk1 は Catalina 以降で iOS 版 bridge に切替 |
...
electron.remote 方針                 ──┘              │
              ...
              FFI 置き換え → remote 置き換え → Electron 更新
                                       │
                                       ▼
              vendor を評価版 SDK に差し替え → arm64 パッケージング
                                       │
                                       ▼
...
- 評価版 SDK の入手、dylib / framework のパス、関数シグネチャ、辞書・phont の差分確認
- vendor への取り込み（開発用）。形式が `.framework` から dylib に変わっている可能性
- `gulpfile.package.js` の unpacked コピー、asar ignore、コード側の `DynamicLibrary` パス
- AquesTalk1 の i386 / iOS 切替を、新 Mac 評価版に合わせるか。機能追加ではなく、現行の音声生成を新バイナリで成立させるため
- `maquestalk1` / `maquestalk1-ios` を、評価版 AquesTalk1（arm64）に合わせて自前でビルドし直す。Electron 本体とは別リポジトリの作業
- `secret`（Go）を arm64 向けにビルドし直す。AquesTalk SDK とは独立。https://github.com/myukkurivoice/myukkurivoice-secret
- サンプリングレート等の仕様差分。現行は 8000Hz 扱い。新 AquesTalk10 マニュアルは 16kHz。実装前に実測する
- 評価版で開発が成立したあと、製品配布に有償ライセンスが必要かは、その時点で決める
...
- AquesTalk は現行 vendor に arm64 がないため更新する。開発は評価版 SDK を使う
...
| 順 | 内容 | 依存 |
| --- | --- | --- |
| 1 | vendor を AquesTalk 評価版 SDK に差し替え。`maquestalk1` 系と `secret` を arm64 でビルド | 調査 1 と 2 |
...
| 領域 | 大きさ | 理由 |
| --- | --- | --- |
| AquesTalk vendor | 中〜大 | 評価版 SDK の形式・API 差分。有償ライセンスは後で判断 |
