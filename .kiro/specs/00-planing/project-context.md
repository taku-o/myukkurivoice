...
**進め方:** これからは **cc-sdd**（`.cursor/skills/kiro-*`）で進める。工程は1つずつ。taku-o が次を言うまで止まる。`/kiro-spec-init` は使わない。`-y` は使わない。`approved` は自分で立てない。調査は済んだ。アクティブな spec は無い。アプリ全体を作らないと確かめられない調査は後回し。工程表の正本は [cc-sdd-adoption.md](./cc-sdd-adoption.md)。

**誰がやるか（2026-09-19）:** Cursor は進捗・仕様・引き渡し。製品実装は Claude Code。Cursor は工程 1–6。Claude Code は工程 7–9。Cursor は製品実装を始めない。`/kiro-review-spec` と `/kiro-review-feature` は Claude Code 専用。Cursor では動かない。
...
確認した先端: `1688ad5`（2026-09-18、prompt.md の評価版パスと調査先行メモ）。planning.md の prompt.md コミット規則は未コミットの追記。resume.md 本文の「先端は `6c2ea99`」は古い。
...
- `maquestalk1` / `maquestalk1-ios` / `secret` は自作。現行は外部コマンド。新版の AT1 は koffi 直呼び。`maquestalk1` も `maquestalk1-ios` も捨てる。`secret` は残す
...
- taku-o の工程指示なしに `requirements.md` / `design.md` / `tasks.md` / `spec.json` を新規作成する
- `spec.json` の `approved` を自己判断で `true` にする
- `-y` と `/kiro-spec-quick --auto` で承認ゲートを飛ばす
- 工程をまとめて進める（requirements のあと gap、design のあと tasks、を自分で続けない）
...
| AquesTalk1 Mac 用 CLI（Xcode） | [myukkurivoice/maquestalk1](https://github.com/myukkurivoice/maquestalk1) | 現行は外部コマンド。新版では捨てる |
...
`docs/development.md` の related links も `myukkurivoice/maquestalk1` と `myukkurivoice/maquestalk1-ios`。resume.md 本文の `taku-o/maquestalk1` は GitHub 上に無く、実体は `myukkurivoice` org。
...
正本は [upcoming-work.md](./upcoming-work.md)。1 から。
...
次の cc-sdd コマンドは決まっていない。THEN 1（`maquestalk1` を捨てて AT1 を koffi 直呼び）は将来の作業。アクティブな spec は無い。feature 名は taku-o が工程 1 を出すときに決める。工程表は [cc-sdd-adoption.md](./cc-sdd-adoption.md)。順は requirements → gap → design → design 検証 → tasks → 人間レビュー（ここまで Cursor）→ 1タスク実装 → その検証 → 実装確認（ここから Claude Code）。各工程のあと taku-o 待ち。`/kiro-spec-init` は使わない。Cursor は製品実装を始めない。実装を頼まれたら引き渡しパック（パス、feature 名、task number、spec ファイル、制約）を作る。
...
| Project store `docs/cc-sdd-adoption.md` | これからは cc-sdd。工程表の正本（init なし、1つずつ止まる。Cursor=1–6、Claude Code=7–9） |
...
resume.md は `.cursor/rules/cursor-local.mdc` も挙げているが、`feature/applecpu/master` 先端にはそのファイルは無い。
