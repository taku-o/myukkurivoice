...
angular.module('yvoiceLicenseService', []).factory('LicenseService', [
  '$q',
  function($q) {
    ...
    return {
      ...
      consumerKey: function(licenseType) {
        ...
        // passPhrase
        exec(secretCmd + ' -key=passPhrase', cmdOptions, function(
          err,
          stdout,
          stderr,
        ) {
          ...
          // licenseKey
          exec(secretCmd + ' -key=' + licenseType, cmdOptions, function(
            err,
            stdout,
            stderr,
          ) {
