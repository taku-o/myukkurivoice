...
angular
  ...
  .factory('AquesService', [
    ...
    function($q, MessageService, LicenseService) {
      ...
      frameworkPath =
        unpackedPath + '/vendor/AqKanji2Koe.framework/Versions/A/AqKanji2Koe';
      var ptr_AqKanji2Koe_Create = ffi
        .DynamicLibrary(frameworkPath)
        .get('AqKanji2Koe_Create');
      var ptr_AqKanji2Koe_Release = ffi
        .DynamicLibrary(frameworkPath)
        .get('AqKanji2Koe_Release');
      var ptr_AqKanji2Koe_Convert = ffi
        .DynamicLibrary(frameworkPath)
        .get('AqKanji2Koe_Convert');
      var fn_AqKanji2Koe_Create = ffi.ForeignFunction(
        ptr_AqKanji2Koe_Create,
        ptr_void,
        ['string', ptr_int],
      );
      var fn_AqKanji2Koe_Release = ffi.ForeignFunction(
        ptr_AqKanji2Koe_Release,
        'void',
        [ptr_void],
      );
      var fn_AqKanji2Koe_Convert = ffi.ForeignFunction(
        ptr_AqKanji2Koe_Convert,
        'int',
        [ptr_void, 'string', ptr_char, 'int'],
      );
      ...
      frameworkPath =
        unpackedPath + '/vendor/AquesTalk2.framework/Versions/A/AquesTalk2';
      var ptr_AquesTalk2_Synthe_Utf8 = ffi
        .DynamicLibrary(frameworkPath)
        .get('AquesTalk2_Synthe_Utf8');
      var ptr_AquesTalk2_FreeWave = ffi
        .DynamicLibrary(frameworkPath)
        .get('AquesTalk2_FreeWave');
      var fn_AquesTalk2_Synthe_Utf8 = ffi.ForeignFunction(
        ptr_AquesTalk2_Synthe_Utf8,
        ptr_uchar,
        ['string', 'int', ptr_int, ptr_void],
      );
      var fn_AquesTalk2_FreeWave = ffi.ForeignFunction(
        ptr_AquesTalk2_FreeWave,
        'void',
        [ptr_uchar],
      );
      ...
      frameworkPath =
        unpackedPath + '/vendor/AquesTalk10.framework/Versions/A/AquesTalk';
      var ptr_AquesTalk10_Synthe_Utf8 = ffi
        .DynamicLibrary(frameworkPath)
        .get('AquesTalk_Synthe_Utf8');
      var ptr_AquesTalk10_FreeWave = ffi
        .DynamicLibrary(frameworkPath)
        .get('AquesTalk_FreeWave');
      var ptr_AquesTalk10_SetDevKey = ffi
        .DynamicLibrary(frameworkPath)
        .get('AquesTalk_SetDevKey');
      var ptr_AquesTalk10_SetUsrKey = ffi
        .DynamicLibrary(frameworkPath)
        .get('AquesTalk_SetUsrKey');
      var fn_AquesTalk10_Synthe_Utf8 = ffi.ForeignFunction(
        ptr_AquesTalk10_Synthe_Utf8,
        ptr_uchar,
        [ptr_AQTK_VOICE, 'string', ptr_int],
      );
      var fn_AquesTalk10_FreeWave = ffi.ForeignFunction(
        ptr_AquesTalk10_FreeWave,
        'void',
        [ptr_uchar],
      );
      var fn_AquesTalk10_SetDevKey = ffi.ForeignFunction(
        ptr_AquesTalk10_SetDevKey,
        'int',
        ['string'],
      );
      var fn_AquesTalk10_SetUsrKey = ffi.ForeignFunction(
        ptr_AquesTalk10_SetUsrKey,
        'int',
        ['string'],
      );
      ...
      return {
        encode: function(source) {
          if (!source) {
            MessageService.syserror(
              '音記号列に変換するメッセージが入力されていません。',
            );
            return '';
          }
          var allocInt = ref.alloc('int');
          var aqKanji2Koe = fn_AqKanji2Koe_Create(
            unpackedPath + '/vendor/aq_dic_large',
            allocInt,
          );
          ...
          if (errorCode != 0) {
            ...
            log.warn(
              'fn_AqKanji2Koe_Create raise error. error_code:' +
                errorTable_AqKanji2Koe(errorCode),
            );
            return '';
          }
          var sourceLength = new Blob([sourceLength], {type: 'text/plain'})
            .size;
          var encodedLength = sourceLength >= 512 ? sourceLength * 4 : 512;
          var buf = Buffer.alloc(sourceLength >= 512 ? sourceLength * 4 : 512);
          var r = fn_AqKanji2Koe_Convert(
            aqKanji2Koe,
            source,
            buf,
            encodedLength,
          ...
          if (r != 0) {
            ...
            log.info(
              'fn_AqKanji2Koe_Convert raise error. error_code:' +
                errorTable_AqKanji2Koe(r),
            );
            return '';
          }
          ...
          if (phont.version == 'talk1') {
            ...
            temp.open(fsprefix, function(err, info) {
              ...
              fs.writeFile(info.path, encoded, function(err) {
                ...
                var waverCmd =
                  unpackedPath.replace(' ', '\\ ') + '/vendor/maquestalk1';
                exec(
                  'cat ' +
                    info.path +
                    ' | VOICE=' +
                  ...
                  if (statusCode < 0) {
                    ...
                    log.info(
                      'AquesTalk_SyntheMV raise error. error_code:' +
                        errorTable_AquesTalk2(errorCode),
                    );
                  }
                }); // maquestalk1
              }); // fs.writeFile
            ...
            fs.readFile(phont.path, function(err, phontData) {
              ...
              var allocInt = ref.alloc('int');
              var r = fn_AquesTalk2_Synthe_Utf8(
                encoded,
                speed,
                allocInt,
                phontData,
              ...
              if (ref.isNull(r)) {
                ...
                MessageService.syserror(errorTable_AquesTalk2(errorCode));
                log.info(
                  'fn_AquesTalk2_Synthe_Utf8 raise error. error_code:' +
                    errorTable_AquesTalk2(errorCode),
                );
                d.reject(null);
                return;
            ...
            LicenseService.consumerKey('aquesTalk10DevKey')
              .then(function(licenseKey) {
                ...
                if (!_isAquesTalk10LicensekeySet) {
                  ...
                  if (encryptedUseKey && !aquesTalk10UseKey) {
                    MessageService.error(
                      'AquesTalk10使用ライセンスキーの復号に失敗しました。環境設定で使用ライセンスキーを設定し直してください',
                    );
                    d.reject(null);
                    return;
                  }
                  if (encryptedUseKey) {
                    ...
                    if (usrKey != 0) {
                      MessageService.error(
                        '' +
                          'AquesTalk10使用ライセンスキーが正しくありません。環境設定で使用ライセンスキーを設定してください。' +
                          aquesTalk10UseKey,
                      );
                      d.reject(null);
                      return;
                ...
                // create wave buffer
                var allocInt = ref.alloc('int');
                var r = fn_AquesTalk10_Synthe_Utf8(
                  ptr_aqtkVoiceVal,
                  encoded,
                  allocInt,
                );
                if (ref.isNull(r)) {
                  ...
                  log.info(
                    'fn_AquesTalk10_Synthe_Utf8 raise error. error_code:' +
                      errorTable_AquesTalk10(errorCode),
                  );
                  d.reject(null);
                  return;
                }
              ...
              ['catch'](function(err) {
                MessageService.syserror(
                  'AquesTalk10開発ライセンスキーの読み込みに失敗しました。',
                  err,
                );
                d.reject(err);
              });
        ...
        record: function(wavFilePath, bufWav, options) {
          ...
          fs.writeFile(wavFilePath, bufWav, function(err) {
            ...
            MessageService.record(
              '' + '音声ファイルを保存しました。path: ' + wavFilePath,
              wavFilePath,
            );
            d.resolve('ok');
          });
        ...
        play: function(bufWav, options, parallel) {
          ...
          audioCtx
            ...
            .then(function(decodedData) {
              // report duration
              AppUtilService.reportDuration(
                decodedData.duration + options.writeMarginMs / 1000.0,
              );
              // source
      ...
      return {
        ...
        save: function(filePath, sourceText) {
          ...
          fs.writeFile(filePath, sourceText, 'utf-8', function(err) {
            ...
            MessageService.info(
              '' + 'メッセージファイルを保存しました。path: ' + filePath,
            );
            d.resolve(filePath);
          });
          return d.promise;
          ...
          fs.readdir(dir, function(err, files) {
            ...
            if (maxNum >= limit) {
              MessageService.syserror(
                '' +
                  limit +
                  'までファイルが作られているので、これ以上ファイルを作成できません。',
              );
              d.reject(null);
              return;
            }
