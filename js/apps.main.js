...
angular
  ...
  .controller('MainController', [
    ...
    function(
      ...
      // application settings
      var AudioService =
        appCfg.audioServVer == 'html5audio' ? audioServVer1 : audioServVer2;
      $scope.appCfg = appCfg;
      // init
      var ctrl = this;
      $scope.display = 'main';
      ...
      ctrl.play = function() {
        ...
        if (!encoded) {
          ...
          if (CommandService.containsCommand(source, $scope.yvoiceList)) {
            var parsedListForEnc = CommandService.parseInput(
              source,
              $scope.yvoiceList,
              $scope.yvoice,
            );
        ...
        // play
        var parsedList = CommandService.parseInput(
          encoded,
          $scope.yvoiceList,
          $scope.yvoice,
        );
      ...
      function playEach(cinput) {
        ...
        var yvoice = CommandService.detectVoiceConfig(
          cinput,
          $scope.yvoiceList,
        );
        // phont
        ...
        if (
          !(Number(yvoice.writeMarginMs) === parseInt(yvoice.writeMarginMs))
        ) {
      ...
      ctrl.record = function() {
        ...
        if ($scope.yvoice.seqWrite) {
          ...
          ipcRenderer.once('showSaveDialog', function(event, filePath) {
            ...
            // record
            var containsCommand = CommandService.containsCommand(
              encoded,
              $scope.yvoiceList,
            );
            var parsedList = CommandService.parseInput(
            ...
            parsedList
              ...
              .then(function(fp) {
                ...
                var sourceFname = AudioSourceService.sourceFname(
                  firstWroteFile,
                );
                AudioSourceService.save(sourceFname, $scope.yinput.source)
                  .then(function() {})
      ...
      function recordSolo(cinput, filePath) {
        ...
        AquesService.wave(encoded, phont, speed, waveOptions)
          .then(function(bufWav) {
            return AudioService.record(filePath, bufWav, playOptions)
              .then(function() {
                d.resolve(filePath);
              })
              ['catch'](function(err) {
      ...
      ctrl.putVoiceName = function() {
        ...
        // top
        if (pos == 0) {
          $scope.yinput[field.id] =
            '' + $scope.yvoice.name + '＞' + field.value;
          ...
          if (field.value.substring(pos - 1, pos) == '\n') {
            $scope.yinput[field.id] =
              ...
              $scope.yvoice.name +
              '＞' +
              field.value.substring(pos, length);
      ...
      ipcRenderer.on('switchAlwaysOnTop', function(event, newflg) {
        ...
        MessageService.info(
          'update alwaysOnTop option ' + (newflg ? 'ON' : 'OFF'),
        );
        $timeout(function() {
          $scope.$apply();
        });
