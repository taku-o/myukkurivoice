...
const tsProjectDebug = ts.createProject('tsconfig.json');
...
gulp.task('all', () => {
    try {
        return gulp.series('format', 'less', 'tsc', 'lint', 'test', 'staging')
    } catch(err) {
        return _notifyError();
    }
});
...
gulp.task('_tsc', () =>
        ...
        .src(['*.ts', 'js/*.ts', 'test/*.ts', 'docs/assets/js/*.ts'], {base: '.'})
                           ...
                           gulp.task('_rm-js', () =>
                           del(['*.js', 'js/*.js', 'test/*.js', '!gulpfile.js', '*.tsbuildinfo'])
                           );
...
gulp.task('_readme:html', gulp.series(gulp.parallel('_readme:html:css', '_readme:html:icns', '_readme:html:images'),
gulp
.src('docs/README.md')
...
gulp.task('_readme:html:css',() =>
        gulp.src(['docs/assets/css/readme-html.css']).pipe(gulp.dest('MYukkuriVoice-darwin-x64/assets/css'))
        );
...
gulp.task('_readme:html:images',() =>
        gulp.src(['docs/assets/images/*']).pipe(gulp.dest('MYukkuriVoice-darwin-x64/assets/images'))
                                       );
...
gulp.task('test-rebuild', () => {
    try {
        return gulp.series('tsc-debug', '_rm-package', '_package-debug', '_unpacked', '_test', '_notify')
    } catch(err) {
        return _notifyError();
    }
});
...
gulp.task('package', () => {
try {
return gulp.series('tsc-debug', '_rm-package', '_package-debug', '_unpacked', '_notify')
} catch(err) {
return _notifyError();
}
});
