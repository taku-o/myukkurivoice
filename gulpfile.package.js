...
function buildPackage(platform, forDebug, cb) {
  let packageCommand = PACKAGER_CMD +
    ` . MYukkuriVoice \
  ...
  // exec command
  exec(packageCommand, (err, stdout, stderr) => {
    cb(err);
  });
}
gulp.task('_package-release', (cb) => {
  buildPackage('darwin', false, cb);
});
gulp.task('_package-release:store', (cb) => {
  buildPackage('mas', false, cb);
});
gulp.task('_package-debug', (cb) => {
  buildPackage('darwin', true, cb);
});
